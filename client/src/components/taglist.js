    ...
    return (
      <div className="tag-list">
        <div className="row">
          ...
          <button onClick={() => setActivePage('askQuestion')}>Ask a Question</button>
        </div>
        <h3>All Tags</h3>

        <div className="tags-grid">
          {tags.map((tag) => (
            <div className="tagNode" key={tag.tid}>
              <Tag tag={tag} onClick={handleClick} />
              <p>{questions.filter(question => question.tagIds.includes(tag.tid)).length} questions</p>
            </div>
          ))}
        </div>
      </div>
