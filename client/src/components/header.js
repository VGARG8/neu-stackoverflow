...
function Header({ onSearch, questions, tags, answers }) {
  ...
  const handleSearch = (e) => {
    if (e.key === "Enter") {
      ...
      const tagsToSearch = tagMatches.map(tag => tag.slice(1, -1).toLowerCase());
      const words = currentSearchTerm.replace(/\[([^\]]+)\]/g, "").split(/\s+/).map(word => word.toLowerCase());
      ...
      let wordMatchedQuestions = questions.filter(question => {
        ...
        const textWords = question.text ? question.text.toLowerCase().split(/\s+/) : [];
        
        return words.some(word => titleWords.includes(word) || textWords.includes(word));
      });
      ...
      const searchResults = questions.filter(question => {
        const questionTags = question.tagIds.map(tagId => tags.find(tag => tag.tid === tagId).name);
        const questionAnswers = question.ansIds.map(ansId => answers.find(answer => answer.aid === ansId).text.toLowerCase());

        const doesTagMatch = tagsToSearch.every(tag => questionTags.includes(tag));
        const doesWordMatchInAnswers = questionAnswers.some(answerText => words.some(word => answerText.includes(word)));
